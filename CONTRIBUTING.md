...
> 💡 For contributing to {SOFTWARE} itself please see [the source code repository](https://github.com/bluerobotics/{SOFTWARE}).
...
1. [Fork this repository](https://github.com/bluerobotics/{SOFTWARE}-docs/fork)
...
> 💡 These can often be found by [searching the software repository](https://github.com/bluerobotics/{SOFTWARE}/issues?q=is%3Apull+label%3Adocs-needed) for pull requests with the `docs-needed` label.

0. Ensure your fork's `latest` and `main` branches are up to date with this repository
1. Branch out from your `latest` branch into a unique branch, ideally with a short name that reflects its purpose
1. Submit a Draft pull request with your changes, and include "Documents bluerobotics/{SOFTWARE}#{PULL-REQUEST}" in the description
...
> 💡 Relevant features can typically be found by looking through the public documentation for features with minimal information, and/or [searching this repository](https://github.com/bluerobotics/{SOFTWARE}-docs/issues?q=is%3Aissue+is%3Aopen+label%3Acontent) for issues with the `content` label, or [searching the software repository](https://github.com/bluerobotics/{SOFTWARE}/issues?q=is%3Apull+is%3Aclosed+label%3Adocs-minimal) for pull requests with the `docs-minimal` label.
...
> 💡 Requested features/fixes can be found by [searching this repository](https://github.com/bluerobotics/{SOFTWARE}-docs/issues?q=is%3Aissue+is%3Aopen+label%3Ainfrastructure) for issues with the `infrastructure` label.
...
1. Branch out from your `main` branch, and make your desired changes
...
1. [Submit pull requests to the content branches](#content-improvements) as relevant, to provide compatibility with and/or make use of the updated infrastructure
...
1. Clone the `main` branch of the repository, so you can build the documentation from it
    - if you're only changing the content, this clone can be from the original (not your fork)
        - `git clone https://github.com/bluerobotics/{SOFTWARE}-docs {SOFTWARE}-docs-base`
    - if you're planning to change the infrastructure, clone from the relevant branch in your fork
        - e.g. `git clone -b BRANCH https://USER-NAME@github.com/USER-NAME/{SOFTWARE}-docs`, where
            - `USER-NAME` is your GitHub username
            - `BRANCH` is the name of the infrastructure branch you're planning to change
1. Clone the content branch you're making changes in (if relevant)
    - e.g. `git clone -b BRANCH https://USER-NAME@github.com/USER-NAME/{SOFTWARE}-docs {SOFTWARE}-docs-content`, where
        - `USER-NAME` is your GitHub username
        - `BRANCH` is the name of the content branch you're planning to change
1. Enter the infrastructure repository and initialise the submodules, to get the theme and the latest content
   ```sh
   cd {SOFTWARE}-docs-base
